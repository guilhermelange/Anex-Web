...
export default function AnimeModal({ animeId, isOpen, onClose }: AnimeModalProps) {
  const { loadAnime, handleEvaluation, handleSetFavorite, anime } = useContext(AnimeContext);
        ...
        <Flex h={400} bg={'whiteAlpha.200'} w={'full'} alignItems={'center'}
          ...
          <Flex borderTopRadius={4} alignItems={'center'} py={4} px={8} w={'full'} h={'full'} bg={'linear-gradient(90deg,#111 0,transparent)'}>
            <Stack
              ...
              <Stack direction='row' spacing={6}>
                <MotionIconButton whileHover={{ scale: 1.05 }} transition={{ duration: 0.1 }} size='lg' borderRadius={'full'} icon={<BiLike />} onClick={() => { handleEvaluation(true) }} colorScheme={anime?.evaluation ? primary : 'gray'} />
                <MotionIconButton whileHover={{ scale: 1.05 }} transition={{ duration: 0.1 }} size='lg' borderRadius={'full'} icon={<BiDislike />} onClick={() => { handleEvaluation(false) }} colorScheme={anime?.evaluation == null || anime?.evaluation ? 'gray' : primary} />
                <Button size='lg' leftIcon={anime?.favorite ? <MdOutlineBookmarkAdded /> : <MdOutlineBookmarkAdd />} colorScheme={anime?.favorite ? primary : "gray"} variant='solid' onClick={() => { handleSetFavorite() }}>
